...
You can use any _S3_ compatible storage service such as _Red Hat OpenShift Data Foundation_ or _AWS S3_.
...
The entire pipeline is here xref:attachment$sample-pipeline-full.py[Sample Pipeline]
...
The following code demonstrates how to submit and trigger a pipeline run from a _OpenShift AI WorkBench_.
...
[NOTE]
====
The Data Science Projects *_Data Connection_* storage is used to store _Output Artifacts_ and _Parameters_ of a pipeline. It is not used to pass large amounts of data between pipeline steps
====

=== Tekton Workspaces

This uses the out of the box approach in Tekton to store and pass data between stages of a _Tekton Pipeline_.
Tekton will create a *Workspace* to share large data files among tasks that run in the same pipeline. These _Workspaces_ will be backed by dynamically created storage volumes which will be removed once the pipeline has completed.

By default the storage is set to *_ReadWriteMany_*, the size is set to *_2Gi_* and uses the storage class called *_kfp-csi-s3_*. 

However this can be changed to suit the target environment needs by settting the following *_Environment Variables_*

. _DEFAULT_ACCESSMODES_
. _DEFAULT_STORAGE_SIZE_
. _DEFAULT_STORAGE_CLASS_

And example of this in the _Sample Pipeline_ is shown below  

[source,python]
include::example$sample-pipeline-full.py[lines=502..505]

=== Volume based data passing method
This approach uses a pre-created OpenShift storage volume ,aka _(PVC)_, to pass data between the pipeline steps.
...
[IMPORTANT]
====
The *_data-volume PVC claim_* needs to exist on the OpenShift cluster before running the pipeline, if not then the _pipeline execution pod_ will fail to deploy and the run will terminate.
====

To pass big data using cloud provider volumes, it's recommended to use the *_volume_based_data_passing_method_*.

== Data Science Pipeline - Hands On Example

We're now going to deploy and run an example DSP pipeline. The use case is offline scoring for _Fraud Detection_.
...
** [ ] xref:attachment$offline_scoring_kfp_pipeline.py[ONNX Model]
** [ ] xref:attachment$live-data.csv.gz[Live Transaction Data]
** [ ] xref:attachment$training-data.csv.gz[Model Training Data]

[%interactive]
* [ ] Uncompress the *_.gz_* files using the _gzip -d _ command. 
...
Download and customise as necessary this xref:attachment$offline_scoring_kfp_pipeline.py[Example Volume definition]
...
If the pipeline run was executed without issue you should see something similar to below:

image::pipeline-finish.png[]

=== Handling Pipeline Errors

In the case that the pipeline failed the UI will be show something similar to the following:

image::pipeline-failure.png[]

Clicking on the *_failed_* node in the Graph and then clicking on the _Details_ tab will present the status of the execution step. 
