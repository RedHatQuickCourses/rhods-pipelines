...
. Download the following two files from the https://github.com/mamurak/os-mlops-artefacts/tree/fraud-detection-model-v0.1[model artifact repository]:
+
--
...
. We will configure a new `Runtime Image` by opening th `Runtime Images` menu from the left toolbar. Select `Create new runtime image` via the plus sign in the top portion of the menu.
+
