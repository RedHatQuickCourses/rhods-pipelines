...
image::../images/dsps-main.png[Data Science Pipelines Menu / Main View]

* *Data Science Pipelines Runs* - When looking for more details, end users explore the runs of their data science pipelines

image::../images/dsps-runs.png[Data Science Pipelines Runs]
...
image::../images/dsps-in-ds-project.png[Data Science Pipelines view within a Data Science Project]
...
image::../images/dsp-run-in-ocp-pipelines.png[OpenShift Pipeline executing a Data Science Pipeline]
...
* *OpenShift Pipelines (based on Tekton)* - It executes each step in the pipeline as an individual container / job and ensures that each containers gets allocated the resources (GPU, CPU, memory) configured/requested by data scientists in Elyra (or via Kubeflow SDK), as long as they are available in the OpenShift cluster.


* *OpenShift GitOps with ArgoCD for AI/ML flows* - While not part of the OpenShift AI but rather as part of OpenShift, it is worth mentioning that once ML models are created, GitOps can be leveraged to push them to other OpenShift instances (including OpenShift Edge devices) for being hosted on model servers for inference and/or embedded in intelligent apps.
